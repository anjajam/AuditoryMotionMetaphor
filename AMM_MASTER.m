...
% set output file
outFile = fullfile(dataDir,'corrVals.txt');
for i = 1:length(subjDirs)
    ct = 0;
    labels = [];
    % set paths
    ...
    if exist(subjDir,'dir')
        ...
        for j = 1:length(metDirs)
            ...
            lhzstats = listdir(fullfile(sessionDir,metDirs{j},indivDir{1},'stats',...
                'zstat*.surf.lh.nii.gz'),'files');
            rhzstats = listdir(fullfile(sessionDir,metDirs{j},indivDir{1},'stats',...
                'zstat*.surf.rh.nii.gz'),'files');
            for k = 1:length(lhzstats)
                lhMet = load_nifti(fullfile(sessionDir,metDirs{j},indivDir{1},'stats',...
                    lhzstats{k}));
                rhMet = load_nifti(fullfile(sessionDir,metDirs{j},indivDir{1},'stats',...
                    rhzstats{k}));
                % Sound
...
fprintf(fid,[labels '\n']);
fclose(fid);
dlmwrite(outFile,corrVals(2:end,:),'-append'); % note the 2 : end, the first subject is bad